...
// prettier-ignore
type ContextType = {
  connectSocket: (socket: T.SocketType, userOId: string, setter: T.Setter<boolean>) => Promise<void>
  disconnectSocket: (socket: T.SocketType) => void
  emitSocket: (socket: T.SocketType, event: string, payload: any) => void
  offSocket: (socket: T.SocketType, event: string) => void
  onSocket: (socket: T.SocketType, event: string, callback: (payload: any) => void) => void
...
export const SocketCallbacksProvider: FC<PropsWithChildren> = ({children}) => {
  ...
  const connectSocket = useCallback(async (socket: T.SocketType, userOId: string, setter: T.Setter<boolean>) => {
    if (!socket) {
  ...
  const disconnectSocket = useCallback((socket: T.SocketType) => {
    if (socket) {
      socket.disconnect()
      setSocket(null)
    }
  ...
  const emitSocket = useCallback((socket: T.SocketType, event: string, payload: any) => {
    if (socket) {
      socket.emit(event, payload)
    }
  }, []) // eslint-disable-line react-hooks/exhaustive-deps

  const offSocket = useCallback((socket: T.SocketType, event: string) => {
    if (socket) {
      socket.off(event)
    }
  }, []) // eslint-disable-line react-hooks/exhaustive-deps

  const onSocket = useCallback((socket: T.SocketType, event: string, callback: (payload: any) => void) => {
    if (socket) {
